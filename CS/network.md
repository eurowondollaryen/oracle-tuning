...
- UDP: 비연결형 통신으로, TCP처럼 확인 작업을 별도로 하지 않는다.
> 효율성을 중요하게 여기는 프로토콜이라 동영상 서비스 같은 곳에 사용된다.
- UDP 데이터그램: UDP 헤더 + 데이터
> UDP를 사용하면 컴퓨터나 네트워크 장비에 데이터를 일괄적으로 보내는 "브로드캐스트"를 할 수 있다.

## 5. 응용 계층
- 응용 계층: OSI 7 Layer의 세션 계층과 표현 계층을 포함
> 클라이언트 - 서버 통신을 하려면, 응용 계층의 프로토콜을 사용해야 한다. (http, ftp, smtp ...)

## 6. SMTP, POP3
- SMTP: 메일 전송에 사용되는 프로토콜, 25번 포트 사용 (PC->서버, 서버->서버)
- POP3: 메일 서버에 있는 메일을 가져오는 데 사용되는 프로토콜, 110번 포트 사용
- 참고) cmd의 ping 명령어는 ICMP(Internet Control Message Protocol) 프로토콜을 사용한다.

## 7. SSID
- SSID: AP의 고유 이름이며, 네트워크 이름, 인증, 암호화, 암호화 키를 설정해야 한다.(그래야 자동으로 설정할 수 있음)
- 무선 AP에 접속하는 과정
  1. 무선 AP가 자기를 알리는 신호(beacon)을 전체적으로, 주기적으로 전송한다.
  2. 클라이언트는 자신의 SSID와 같은지 무선 AP에 문의한다.
